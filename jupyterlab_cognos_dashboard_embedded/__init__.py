import json, requests, csv, codecs, os
    ...
    def post(self, path=''):
        ...
        data = {
                'dashboard': {
                    ...
                    'cognos_root_url': body['cognos_root_url'],
                    'user': 'Anonymous',
                    'dashboard_spec': body['dashboard_spec']
                }
            }

        res = requests.post(os.environ.get("SHOWCASE_URL")+'/api/dashboards', auth=HTTPBasicAuth(os.environ.get('SHOWCASE_ADMIN_USERNAME'), os.environ.get('SHOWCASE_ADMIN_PASSWORD')), data=json.dumps(data), headers=headers)
...
class CognosDashboardApiHandler(APIHandler):
    ...
    def post(self, path='/sessionCode'):
        ...
        if not api_endpoint_url:
            ...
            res = requests.post('{}v1/session'.format(api_endpoint_url), auth=HTTPBasicAuth(client_id, client_secret), data=json.dumps(data), headers=headers)
    ...
    def put(self, path='/credentials'):
        ...
        api_endpoint_url = json.loads(self.request.body)['credentials'].get('api_endpoint_url','')
        client_id = json.loads(self.request.body)['credentials'].get('client_id','')
        client_secret = json.loads(self.request.body)['credentials'].get('client_secret','')
...
def load_jupyter_server_extension(nb_server_app):
    ...
    handlers = [(endpoint + "(.*)", CognosDashboardApiHandler), (url_path_join(base_url, 'share-cde') + "(.*)", CognosDashboardGalleryApiHandler)]
